...
let actions = `Поменять карту состояния здоровья на новую из колоды.
...
let phobias = `Аблутофобия – боязнь перед плаванием
...
let globalEquips = `Аптечка для душевно больных
Аптечка с антибиотиками
Аптечка с литием
Аптечка с инсулином
Капсула криосна
Огнетушитель
...
Медицинский атлас
Сборник передач Малышевой «Жить здорово!»`.split('\n')

let bunkerArea = `20 кв.м
50 кв.м
100 кв.м
300 кв.м
500 кв.м`.split('\n')

let survivingTime = `6 месяцев
1 год
3 года
5 лет`.split('\n')

let health_statuses_percent = `10%
30%
70%
100%`.split('\n')

let bunkerRooms = `Бибилотека
Кухня
Теплица
Тренажёрный зал
...
Медицинский кабинет
Склад оружия
Комната отдыха`.split('\n')

let globalSurvivorCount = `1%
3%
10%
30%`.split('\n')